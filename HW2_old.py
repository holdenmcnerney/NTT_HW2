...
def dd_pseudo_position(icp_data_rover: dict, ref_svs: np.array, sv_list: list):
    ...
    for time in ref_svs[:, 0]:
        ...
        ref_location_idx = np.absolute(icp_data_rover[ref_svs[ref_idx, 1]][:, 1] - time).argmin()
        ref_location_ned = icp_data_rover[ref_svs[ref_idx, 1]][ref_location_idx, 2:5]

        pseudorange_vec = np.array([0])
        sv_location_vec = np.array([0, 0, 0])
        ...
        if len(visible_sv) >= 4:
            ...
            for sv in visible_sv:
                ...
                current_sv_idx = np.absolute(icp_data_rover[sv][:, 1] - time).argmin()
                pseudorange = icp_data_rover[sv][current_sv_idx, 0]
                sv_location_ned = icp_data_rover[sv][current_sv_idx, 2:5]


                pseudorange_vec = np.vstack((pseudorange_vec, pseudorange))
                sv_location_vec = np.vstack((sv_location_vec, sv_location_ned))
                pass
        
        pseudorange_vec = np.delete(pseudorange_vec, 0, axis=0)
        sv_location_vec = np.delete(sv_location_vec, 0, axis=0)
